...
struct ArtisticBoldTemplate: SimpleTemplateRenderer {
    ...
    func draw(in context: CGContext, page: CGRect, invoice: Invoice, company: Company, customer: Customer, currency: String, logo: UIImage?) {
        ...
        // Bold header
        drawBoldHeader(context: context, page: page, company: company, logo: logo, primary: primary, secondary: secondary, accent: accent)
        
        // Artistic invoice section
        drawArtisticInvoiceSection(context: context, page: page, invoice: invoice, primary: primary, accent: accent)
        
        // Bold bill to section
        drawBoldBillTo(context: context, page: page, customer: customer, primary: primary, accent: accent)
        
        // Artistic table
        drawArtisticTable(context: context, page: page, invoice: invoice, currency: currency, primary: primary, secondary: secondary, accent: accent)
        
        // Bold total
        drawBoldTotal(context: context, page: page, invoice: invoice, currency: currency, primary: primary, accent: accent)
        
        // Artistic footer
        drawArtisticFooter(context: context, page: page, company: company, primary: primary)
    }
    
    private func drawBoldHeader(context: CGContext, page: CGRect, company: Company, logo: UIImage?, primary: UIColor, secondary: UIColor, accent: UIColor) {
        // Bold background with artistic pattern
        ...
        // Artistic bold elements
        context.setFillColor(accent.withAlphaComponent(0.3).cgColor)
        for i in 0..<7 {
            let rect = CGRect(x: CGFloat(i * 90), y: 20, width: 45, height: 45)
            context.fillEllipse(in: rect)
        }
        
        // Logo with bold frame
        if let logo = logo {
            let logoRect = CGRect(x: 45, y: 40, width: 85, height: 85)
            // Bold frame
            context.setStrokeColor(accent.cgColor)
        ...
        let companyFont = UIFont.systemFont(ofSize: 34, weight: .black)
        let companyAttributes: [NSAttributedString.Key: Any] = [
            .font: companyFont,
            .foregroundColor: primary
        ]
        ...
        // Bold tagline
        let taglineFont = UIFont.systemFont(ofSize: 18, weight: .bold)
        let taglineAttributes: [NSAttributedString.Key: Any] = [
            .font: taglineFont,
            .foregroundColor: accent
        ]
        let taglineText = NSAttributedString(string: "ARTISTIC SERVICES", attributes: taglineAttributes)
        let taglineRect = CGRect(x: 150, y: 95, width: page.width - 180, height: 25)
        taglineText.draw(in: taglineRect)
    }
    
    private func drawArtisticInvoiceSection(context: CGContext, page: CGRect, invoice: Invoice, primary: UIColor, accent: UIColor) {
        // Artistic invoice box
        let invoiceRect = CGRect(x: page.width - 240, y: 55, width: 190, height: 75)
        ...
        // Invoice title with artistic styling
        let titleFont = UIFont.systemFont(ofSize: 26, weight: .black)
        let titleAttributes: [NSAttributedString.Key: Any] = [
            .font: titleFont,
            .foregroundColor: accent
        ]
        let titleText = NSAttributedString(string: "INVOICE", attributes: titleAttributes)
        let titleRect = CGRect(x: page.width - 230, y: 65, width: 170, height: 30)
        titleText.draw(in: titleRect)
        
        // Invoice number with artistic styling
        ...
        let numberText = NSAttributedString(string: "#\(invoice.number)", attributes: numberAttributes)
        let numberRect = CGRect(x: page.width - 230, y: 100, width: 170, height: 25)
        numberText.draw(in: numberRect)
        
        // Date with artistic styling
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "MMM dd, yyyy"
        let dateText = NSAttributedString(string: dateFormatter.string(from: invoice.issueDate), attributes: numberAttributes)
        let dateRect = CGRect(x: page.width - 230, y: 125, width: 170, height: 25)
        dateText.draw(in: dateRect)
    }
    
    private func drawBoldBillTo(context: CGContext, page: CGRect, customer: Customer, primary: UIColor, accent: UIColor) {
        // Bold bill to section
        let billToRect = CGRect(x: 45, y: 180, width: 400, height: 120)
        ...
        // Bill to title with bold styling
        let billToFont = UIFont.systemFont(ofSize: 20, weight: .black)
        let billToAttributes: [NSAttributedString.Key: Any] = [
            .font: billToFont,
            .foregroundColor: primary
        ]
        let billToText = NSAttributedString(string: "BILL TO:", attributes: billToAttributes)
        let billToTextRect = CGRect(x: 55, y: 190, width: 120, height: 30)
        billToText.draw(in: billToTextRect)
        
        // Customer details with bold styling
        let customerFont = UIFont.systemFont(ofSize: 18, weight: .bold)
        let customerAttributes: [NSAttributedString.Key: Any] = [
            .font: customerFont,
            .foregroundColor: UIColor.black
        ]
        ...
        let customerNameText = NSAttributedString(string: customer.name, attributes: customerAttributes)
        let customerNameRect = CGRect(x: 55, y: yOffset, width: 380, height: 25)
        customerNameText.draw(in: customerNameRect)
        yOffset += 25
        
        if !customer.address.oneLine.isEmpty {
            let addressText = NSAttributedString(string: customer.address.oneLine, attributes: customerAttributes)
            let addressRect = CGRect(x: 55, y: yOffset, width: 380, height: 25)
            addressText.draw(in: addressRect)
            yOffset += 25
        }
        ...
        if !customer.email.isEmpty {
            let emailText = NSAttributedString(string: customer.email, attributes: customerAttributes)
            let emailRect = CGRect(x: 55, y: yOffset, width: 380, height: 25)
            emailText.draw(in: emailRect)
        }
    }
    
    private func drawArtisticTable(context: CGContext, page: CGRect, invoice: Invoice, currency: String, primary: UIColor, secondary: UIColor, accent: UIColor) {
        let startY: CGFloat = 340
        
        // Artistic table header
        let headerRect = CGRect(x: 45, y: startY, width: page.width - 90, height: 45)
        ...
        // Header text
        let headerFont = UIFont.systemFont(ofSize: 16, weight: .black)
        let headerAttributes: [NSAttributedString.Key: Any] = [
            .font: headerFont,
            .foregroundColor: UIColor.white
        ]
        ...
        let headers = ["Description", "Quantity", "Unit Price", "Total"]
        let columnWidths: [CGFloat] = [370, 100, 120, 120]
        ...
        // Artistic table rows
        let rowFont = UIFont.systemFont(ofSize: 15, weight: .bold)
        let rowAttributes: [NSAttributedString.Key: Any] = [
            .font: rowFont,
            .foregroundColor: UIColor.black
        ]
        ...
        for (index, item) in invoice.items.enumerated() {
            ...
            if index % 2 == 0 {
                let rowRect = CGRect(x: 45, y: yOffset, width: page.width - 90, height: 40)
    ...
    private func drawBoldTotal(context: CGContext, page: CGRect, invoice: Invoice, currency: String, primary: UIColor, accent: UIColor) {
        let totalY: CGFloat = 520
        
        // Bold total box
        let totalRect = CGRect(x: page.width - 270, y: totalY, width: 210, height: 70)
        ...
        // Total text with bold styling
        let totalFont = UIFont.systemFont(ofSize: 22, weight: .black)
        let totalAttributes: [NSAttributedString.Key: Any] = [
            .font: totalFont,
            .foregroundColor: UIColor.white
        ]
        let totalText = NSAttributedString(string: "TOTAL: \(currency)\(String(format: "%.2f", Double(truncating: invoice.subtotal as NSDecimalNumber)))", attributes: totalAttributes)
        let totalTextRect = CGRect(x: page.width - 260, y: totalY + 22, width: 190, height: 25)
        totalText.draw(in: totalTextRect)
    }
    
    private func drawArtisticFooter(context: CGContext, page: CGRect, company: Company, primary: UIColor) {
        let footerFont = UIFont.systemFont(ofSize: 13, weight: .bold)
        let footerAttributes: [NSAttributedString.Key: Any] = [
            .font: footerFont,
            .foregroundColor: UIColor.gray
        ]
        
        let footerText = NSAttributedString(string: "Bold artistic services. Creative excellence delivered.", attributes: footerAttributes)
